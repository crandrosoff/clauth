...
//! [--profile <name>]`, `clauth info <id|latest>`, and `clauth sessions swap
//! <sid> <profile>` — the CLI surface over the session index
...
//! `swap` is the exception: it touches no transcript at all, only the
...
/// `clauth sessions swap <sid> <profile>` — point a live session at another
/// profile.
///
...
/// executor is the safety gate — it performs the swap, or refuses it with a
/// logged reason, exactly as it treats the chain's intents.
pub(crate) fn run_swap(sid: &str, profile: &str) -> Result<()> {
    let row = crate::live_sessions::get(sid)
        .ok_or_else(|| anyhow::anyhow!("no live session '{sid}'\nsee `clauth sessions`"))?;
    ...
    if row.harness == crate::harness::Harness::Codex {
        anyhow::bail!("session '{sid}' is a codex session; swap is claude-only");
    }
    ...
    outln!("{}", swap_receipt(sid, canonical.as_str()));
    Ok(())
}
...
/// wording for when a swap lands.
fn swap_receipt(sid: &str, profile: &str) -> String {
    format!(
        "clauth: pointed session '{sid}' at '{profile}'\n\
         the swap lands at the session's next request, never before it — \
