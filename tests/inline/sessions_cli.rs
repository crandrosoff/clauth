...
// ── clauth sessions swap <sid> <profile> ────────────────────────────────────
//
...
#[test]
fn swap_refuses_an_unknown_sid_with_the_listing_hint() {
    let _sb = HomeSandbox::new();
    ...
    for sid in ["nope", "999999-9"] {
        let err = run_swap(sid, "work").expect_err("an unknown sid must be refused");
        assert_eq!(
            err.to_string(),
...
#[test]
fn swap_refuses_a_codex_session_row() {
    let _sb = HomeSandbox::new();
    ...
    let err = run_swap("4242-0", "spare").expect_err("a codex row must be refused");
    assert_eq!(
        err.to_string(),
        "session '4242-0' is a codex session; swap is claude-only"
    );
    assert_eq!(crate::exit_code(Err(err)), 1);
...
#[test]
fn swap_refuses_an_unknown_profile() {
    let _sb = HomeSandbox::new();
    seed_profiles(&["work"], &[]);
    let _marker = live_claude_row("4242-0", "work");

    let err = run_swap("4242-0", "nope").expect_err("an unknown profile must be refused");
...
#[test]
fn swap_points_the_row_at_the_named_profile_through_the_registry_seam() {
    let _sb = HomeSandbox::new();
    seed_profiles(&["work", "spare"], &[]);
    let _marker = live_claude_row("4242-0", "work");

    run_swap("4242-0", "spare").expect("the swap records the intent");
...
#[test]
fn swap_receipt_names_when_the_move_lands() {
    assert_eq!(
        swap_receipt("4242-0", "spare"),
        "clauth: pointed session '4242-0' at 'spare'\nthe swap lands at the session's next \
         request, never before it — a refused move is logged and the session stays put"
    );
}

#[test]
fn swap_is_a_no_op_for_a_session_already_on_the_profile() {
    let _sb = HomeSandbox::new();
    seed_profiles(&["work"], &[]);
    let _marker = live_claude_row("4242-0", "work");

    run_swap("4242-0", "work").expect("already-on is a satisfied ask, not an error");
...
#[test]
fn swap_leaves_the_go_no_go_with_the_sessions_executor() {
    let _sb = HomeSandbox::new();
    seed_profiles(&["work"], &[]);
...
#[test]
fn the_swap_grammar_nests_under_sessions_and_keeps_the_bare_listing() {
    use clap::Parser as _;

    let bare =
        crate::cli::Cli::try_parse_from(["clauth", "sessions"]).expect("bare sessions parses");
    match bare.command {
        Some(crate::cli::Command::Sessions { cmd: None, .. }) => {}
        other => panic!("bare `clauth sessions` must stay the listing: {other:?}"),
    }

    let swap = crate::cli::Cli::try_parse_from(["clauth", "sessions", "swap", "4242-0", "spare"])
        .expect("swap parses");
    match swap.command {
        Some(crate::cli::Command::Sessions {
            cmd: Some(crate::cli::SessionsCommand::Swap { sid, profile }),
            ..
        }) => {
            assert_eq!(sid, "4242-0");
            assert_eq!(profile, "spare");
        }
        other => panic!("`sessions swap <sid> <profile>` must parse: {other:?}"),
    }

    // The listing's flags belong to the bare form; mixing one with the verb is
    // a usage error, never a listing that silently ignores the verb.
    let err = crate::cli::Cli::try_parse_from([
        "clauth", "sessions", "--json", "swap", "4242-0", "spare",
    ])
    .expect_err("flags conflict with the subcommand");
    assert_eq!(err.exit_code(), 2);
}
